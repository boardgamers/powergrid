...
After real resource buying, finish a simulated building and/or powering phase with **Done**, then select **Validate**. Each queued phase includes its ending pass. A phase already due plays immediately when validated; later phases wait for the player's turn. The Plan button shows the number of saved premoves and reopens them on the board, where Undo edits the preview and Cancel premoves clears the saved queue, including after reload, and execution does not require an open browser.
