    ...
    for (const width of [390, 1400]) {
        ...
        const regions = await page.locator('.region-border').evaluateAll((elements) =>
            elements.map((element) => ({
                ...
                pattern: element.getAttribute('stroke-dasharray'),
            }))
        );
        assert.equal(regions.length, state.map.cities.length);
        const patterns = new Map();
        for (const { region, pattern } of regions) {
            if (patterns.has(region)) assert.equal(pattern, patterns.get(region));
            patterns.set(region, pattern);
        }
        assert.equal(new Set(patterns.values()).size, new Set(state.map.cities.map((city) => city.region)).size);
        assert.equal(await page.locator('.region-borders text').count(), 0, 'regions need no letter labels');
        ...
        assert.equal(await page.locator('.region-borders').count(), 0);
